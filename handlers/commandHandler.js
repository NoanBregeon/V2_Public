...
class CommandHandler {
  ...
  async initialize() {
    ...
    try {
      ...
      if (forceKill) {
        ...
        console.log(
          `🔍 Live ping startup: enabled=${notificationConfig.isLivePingEnabled()} role=${all.effectivePingRoleId || 'none'}`
        );
      }
    } catch (e) {
  ...
  _defineCommands() {
    const defs = [
      ...
      [
        ...
        [
          { name: 'query', type: 3, description: 'Texte (>=2)', required: true },
          {
            name: 'dans',
            type: 3,
            description: 'Scope (all/mods/vips/bans)',
            required: false,
            choices: [
      ...
      [
        'liveping',
        'Gérer le ping des notifications live',
        [
          {
            name: 'action',
          ...
          {
            name: 'message',
            description: 'Message de ping personnalisé (placeholders {streamer},{title})',
            type: 3, // STRING
            required: false
          }
        ]
  ...
  async handleInteraction(interaction) {
    ...
    try {
      const modGroup = [
        'addmodo','removemodo','addvip','removevip',
  ...
  async handleWelcomeTest(interaction) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '👋 Message d\'arrivée', value: `Bienvenue ${member} sur le serveur !`, inline: false },
        { name: '👋 Message de départ', value: `${member.displayName} a quitté le serveur.`, inline: false },
        { name: 'État du système', value: '🟡 En développement', inline: true },
        { name: 'Canal', value: `<#${interaction.channel.id}>`, inline: true }
      )
      .setColor(0x3498DB)
