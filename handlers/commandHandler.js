...
// Fix imports / fetch fallback (corrige typo 'nodefetch')
...
class CommandHandler {
    ...
    async initialize() {
        // --- Garantie : forcer l'état "live ping" au démarrage si demandé ---
        try {
            const notificationConfig = require('../modules/utils/notificationConfig');

            const envKillRaw = (process.env.FORCE_DISABLE_PINGS || '').toString().toLowerCase();
            const forceKill = envKillRaw === 'true' || envKillRaw === '1';

            const envRaw = (process.env.LIVE_PING_ENABLED || '').toString().toLowerCase();
            const envFalse = envRaw === 'false' || envRaw === '0';

            if (forceKill) {
                // Kill switch total : forcer off, vider role/message en .env et sauvegarder
                ...
                try { process.env.LIVE_PING_ROLE_ID = ''; process.env.LIVE_PING_MESSAGE = ''; } catch {}
                console.log('🔕 FORCE_DISABLE_PINGS=1 détecté → live-ping forcé OFF (persisté)');
            } else if (envFalse) {
                // Si .env demande false : appliquer et persister (empêche réactivation au boot)
                notificationConfig.setLivePing(false);
                notificationConfig.setPingRoleId(null);
                notificationConfig.setPingMessage(null);
                notificationConfig.reload();
                try { process.env.LIVE_PING_ROLE_ID = ''; process.env.LIVE_PING_MESSAGE = ''; } catch {}
                console.log('🔕 LIVE_PING_ENABLED=false détecté → live-ping désactivé et persisté');
            } else {
                // Sinon recharger pour s'assurer qu'on lit la config la plus récente
                notificationConfig.reload();
                const all = notificationConfig.getAll();
                console.log(`🔍 Live ping startup: enabled=${notificationConfig.isLivePingEnabled()} hardKill=${all.hardKill || false} role=${all.effectivePingRoleId || 'none'}`);
            }
        } catch (e) {
            console.warn('⚠️ Impossible d\'initialiser override live-ping au démarrage:', e.message);
        }
    ...
    _defineCommands() {
        const defs = [
            ...
            ['ping','Latence',[]],
            ['help','Aide',[]],
            ['clear','Supprimer des messages',[{name:'nombre',type=4,description:'1100',required=true,min_value=1,max_value=100}]],
            ...
            ['twitchtimeout','Timeout Twitch (sec)',[
                {name:'username',type=3,description:'Pseudo Twitch',required=true},
                {name:'duration',type=4,description:'Durée (601209600)',required=true,min_value=60,max_value=1209600},
                {name:'reason',type=3,description:'Raison',required=false}
            ]],
            ...
            ['reloadcommands','Recharger les commandes slash (admin)',[]],
             ['liveping','Gérer le ping des notifications live',[
                 {
                     name: 'action',
                 ...
                 {
                     name: 'role',
                     description: 'Rôle à ping (optionnel quand activation)',
                     type: 8, // ROLE
                     required: false
                 }
                ,
                {
                    name: 'message',
                    description: 'Message de ping personnalisé (utilise placeholders {streamer},{title})',
                    type: 3, // STRING
                    required: false
                }
             ]],
    ...
    async _registerSlash() {
        // Par défaut on évite d'enregistrer ici pour ne pas dupliquer les commandes.
        // Pour forcer l'enregistrement depuis ce handler, exportez ENABLE_HANDLER_COMMANDS=true dans l'environnement.
        if (process.env.ENABLE_HANDLER_COMMANDS !== 'true') {
            console.log('ℹ️ Enregistrement des commandes du commandHandler SKIPPÉ (ENABLE_HANDLER_COMMANDS != true)');
            return;
        }
    ...
    // Ajout d'un wrapper public attendu par le bootstrap (index.js)
    // Usage : commandHandler.registerSlashCommands() --> délègue à _registerSlash()
    async registerSlashCommands(options = {}) {
		try {
			// possibilité future d'utiliser options.force si nécessaire
			await this._registerSlash();
			console.log('✅ registerSlashCommands exécuté');
		} catch (err) {
			console.error('❌ Échec registerSlashCommands:', err);
			// ne pas relancer pour éviter crashs non gérés lors du bootstrap
		}
	}
    ...
    async handleInteraction(interaction) {
        ...
        // Commande admin pour recharger les commandes slash
        if (cmd === 'reloadcommands') {
            ...
            try {
                ...
                await this._registerSlash();
                return interaction.editReply('✅ Commandes slash rechargées.');
            } catch (e) {
                console.error('❌ Erreur reloadcommands:', e);
                return interaction.reply({ content: `❌ Échec rechargement: ${e.message}`, ephemeral: true });
            }
        }

        // Gestion /liveping (admin) - implementation propre
        if (cmd === 'liveping') {
            if (!interaction.member.permissions.has(PermissionFlagsBits.Administrator)) {
                return interaction.reply({ content: '❌ Tu dois être administrateur pour utiliser cette commande.', ephemeral: true });
            }
            ...
            try {
                if (action === 'status') {
                    ...
                    const source = envMsg ? 'ENV' : (savedMsg ? 'CONFIG' : 'TEMPLATE');
                    const shownMsg = envMsg || savedMsg || (this.templates?.templates?.liveNotification?.ping || '');
                    return interaction.reply({
                        content: `🔔 Live ping: **${enabled ? 'activé' : 'désactivé'}**\n• Source message: **${source}**\n• Message actuel: ${shownMsg ? `\`${shownMsg}\`` : '_aucun_'}${roleId ? `\n• Rôle ping: <@&${roleId}>` : ''}`,
                        ephemeral: true
                    });
                }
                ...
                if (action === 'enable') {
                    ...
                    return interaction.reply({ content: `✅ Live ping activé${role ? ` — rôle ping: ${role}` : ''}${messageOption ? ' — message sauvegardé' : ''}`, ephemeral: true });
                }
    ...
    async handleMessage(message) {
        // Déprécier les commandes préfixées : rediriger vers slash quand possible
        if (!message.content.startsWith(this.prefix) || message.author.bot) return;

        const args = message.content.slice(this.prefix.length).trim().split(/\s/);
        const cmd = args.shift()?.toLowerCase();

        // Liste de commandes legacy que l'on permet encore d'exécuter via message
        const voiceCmds = ['rename','limit','lock','unlock','transfer'];

        // Si commande vocale legacy, appeler le handler existant puis prévenir de la migration vers slash
        if (voiceCmds.includes(cmd)) {
            const voice = this.client.moduleManager?.getModule('voiceManager');
            if (!voice) {
                await message.reply('❌ Module vocal indisponible. Utilisez les commandes slash (/rename, /limit, ...).');
                return;
            }
            // Appel au handler texte (conserve logique actuelle)
            await voice.handleTextCommand(message, cmd, args);

            // Message de dépréciation (autosuppression après 8s)
            try {
                const warn = await message.channel.send(`⚠️ La commande \`!${cmd}\` est dépréciée — utilise désormais la commande slash \`/${cmd}\`.`);
        ...
        // Pour toute autre commande en préfixe : inviter à utiliser la commande slash si elle existe
        if (this.commands.has(cmd)) {
            try {
                const info = await message.reply(`⚠️ Les commandes en \`!\` sont dépréciées. Utilise la commande slash \`/${cmd}\` à la place.`);
        ...
        // Commande inconnue
        // (Ne pas spammer : réponse courte)
        try {
            const r = await message.reply('❌ Commande inconnue. Utilise les commandes slash (`/`) pour interagir avec le bot.');
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                },
                {
                    name: '🔧 Utilitaires',
                    value: [
                        '`/clear <nombre>`  Supprimer des messages (1100)',
    ...
    async _handleWelcome(interaction) {
        ...
        switch (sub) {
            ...
            default:
                return interaction.reply({ content:'❌ Souscommande inconnue', ephemeral:true });
        }
    }
    ...
    // === Remplacement des helpers Twitch (corrigés) ===
    _twitchHeaders() {
        return {
    ...
    async _twitchAddRemove(userType, action, login) {
        ...
        const base = userType === 'moderator' ? 'moderation/moderators' : 'channels/vips';
