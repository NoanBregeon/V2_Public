...
function missing() {
  ...
  return miss.length ? `Twitch non configuré: ${miss.join(', ')}` : null;
}
...
async function addMod(login) {
  ...
  await axios.post('https://api.twitch.tv/helix/moderation/moderators', null, {
    params: { broadcaster_id, user_id: u.id },
...
async function addVip(login) {
  ...
  if (!u) throw new Error('Utilisateur Twitch introuvable.');
  const broadcaster_id = await resolveBroadcasterId();
  await axios.post('https://api.twitch.tv/helix/channels/vips', null, {
    params: { broadcaster_id, user_id: u.id },
    headers: helixHeaders()
  });
  return u;
...
async function listMods() {
  const broadcaster_id = await resolveBroadcasterId();
  const r = await axios.get('https://api.twitch.tv/helix/moderation/moderators', {
    params: { broadcaster_id, first: 100 },
    headers: helixHeaders()
  });
  return r.data?.data ?? [];
}
async function listVips() {
  const broadcaster_id = await resolveBroadcasterId();
  const r = await axios.get('https://api.twitch.tv/helix/channels/vips', {
    params: { broadcaster_id, first: 100 },
    headers: helixHeaders()
  });
  return r.data?.data ?? [];
...
module.exports = [
  {
    ...
    meta: { adminOnly: true, guildOnly: true, cooldownMs: 4000 },
    async execute(i) {
      const miss = missing();
      await safeExec(i, async () => {
        if (miss) return miss;
        const login = i.options.getString('username', true);
  ...
  },
  {
    data: new SlashCommandBuilder()
      ...
      .setDMPermission(false),
    meta: { adminOnly: true, guildOnly: true, cooldownMs: 2000 },
    async execute(i) {
      const miss = missing();
      await safeExec(i, async () => {
        if (miss) return miss;
